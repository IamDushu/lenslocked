...
import (
	...
	"github.com/iamDushu/lenslocked/rand"
)
...
type PasswordReset struct {
	ID     int
	UserID int
	// Token is only set when a PasswordReset is being created;i.e only TokenHash is stored in DB.
	Token     string
	TokenHash string
	//We are using time.Time and not sql.NullTime cuz ExpiresAt is NOT NULL
	ExpiresAt time.Time
}
